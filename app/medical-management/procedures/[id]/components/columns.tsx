...
import { PurchaseOrderItem } from '../../schemas/procedures';
...
export const columns: ColumnDef<PurchaseOrderItem & { currency: string }>[] = [
    ...
    cell: ({ row }) => {
      return (
        <span className="font-medium">
          {row.original.currency} {row.original.price_unit.toFixed(2)}
        </span>
      );
    },
  },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="font-medium">
        {row.original.currency} {row.original.price_subtotal.toFixed(2)}
      </span>
    ),
  },
];
