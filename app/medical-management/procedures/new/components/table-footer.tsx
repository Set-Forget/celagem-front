...
export default function TableFooter({
    ...
    const { control } =
        useFormContext<z.infer<typeof newPurchaseOrderSchema>>();
    ...
    const taxes = items.reduce((acc, item) => {
        return (
            acc + item.quantity * Number(item.price) * (Number(item.tax) / 100)
        );
    }, 0);
    ...
    return (
        <ShadcnTableFooter className="border-t-0">
            <TableRow className="bg-background">
                ...
                <TableCell
                    colSpan={1}
                    className="h-6 text-xs font-medium py-0 text-right pr-5"
                >
