...
const UnitPriceCell = ({ control, index }: { control: Control<z.infer<typeof newBillSchema>>; index: number }) => {
  ...
  const currency = currencies?.data.find((c) => c.id === currencyId)?.name
...
const SubtotalCell = ({ control, index }: { control: Control<z.infer<typeof newBillSchema>>; index: number }) => {
  const { data: currencies } = useListCurrenciesQuery()
...
const TaxesCell = ({ control, index }: { control: Control<z.infer<typeof newBillSchema>>; index: number }) => {
...
const QuantityCell = ({ control, index }: { control: Control<z.infer<typeof newBillSchema>>; index: number }) => {
  return <FormField
    control={control}
    name={`items.${index}.quantity`}
    render={({ field }) => (
...
const CostCenterCell = ({ control, index }: { control: Control<z.infer<typeof newBillSchema>>; index: number }) => {
...
const AccountingAccountCell = ({ control, index }: { control: Control<z.infer<typeof newBillSchema>>; index: number }) => {
