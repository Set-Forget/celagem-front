...
export default function Page() {
  ...
  const { data: paymentTermData, isLoading: isPaymentTermLoading } =
    useGetPaymentTermQuery(paymentTermId);

  const [tab, setTab] = useState('tab-1');

  return (
    <>
      <Header title="Detalles del término de pago">
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button
            onClick={() => router.push(`/accounting/extras/payment-terms/edit/${paymentTermId}`)}
            size="sm"
          >
        ...
        >
          ...
          <div className="p-4 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              >
                {isPaymentTermLoading ? placeholder(13) : 'Hace 3 días'}
              </span>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-muted-foreground text-sm">
                ...
                >
                  {isPaymentTermLoading
                    ? placeholder(13)
                    : paymentTermData?.updated_by?.first_name +
                      ' ' +
                      paymentTermData?.updated_by?.last_name}
                </span>
              </label>
              <span
                className={cn(
