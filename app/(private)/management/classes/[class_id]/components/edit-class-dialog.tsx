...
import { z } from "zod";
import { newClassSchema, Classes, classUpdateBodySchema } from "../../schema/classes";

export default function EditClass() {
  const [dialogState, setDialogState] = useState<DialogsState>({ open: false })

  const role = dialogState.payload?.role as Classes;

  const editRoleForm = useForm<z.infer<typeof newClassSchema>>({
    ...
    <Dialog
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Edita una clase para asignar a tus usuarios.
          </DialogDescription>
        </DialogHeader>
        <Form {...editRoleForm}>
