...
const customers = [
  { id: "PROV-1", name: "Juan Pérez" },
  { id: "PROV-2", name: "María González" },
  { id: "PROV-3", name: "Carlos Ramírez" },
  { id: "PROV-4", name: "Ana López" },
  { id: "PROV-5", name: "Luis Fernández" },
  { id: "PROV-6", name: "Sofía Martínez" },
  { id: "PROV-7", name: "Pedro Torres" },
  { id: "PROV-8", name: "Lucía Morales" }
];

export default function ItemRow({ index, remove }: { index: number, remove: (index: number) => void }) {
  const { control, setValue } = useFormContext<z.infer<typeof newReceiptSchema>>()
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {filtered_bills.map((bill, bills_index) => {
                          ...
                          const provider = customers.find((supplier) => supplier.id === bill.provider)?.name
                          return (
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <FormControl>
                    ...
                    >
                      {field.value ? customers.find((supplier) => supplier.id === field.value)?.name : "Selecciona un cliente"}
                      <ChevronsUpDown className="opacity-50" />
                    </Button>
                  </FormControl>
                </PopoverTrigger>
