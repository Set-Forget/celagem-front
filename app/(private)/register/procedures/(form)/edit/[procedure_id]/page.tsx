...
import {
  newProcedureGeneralSchema,
  newProcedureJobPositionsSchema,
  newProcedureMaterialsSchema,
  newProcedureMedicalExamsSchema,
  newProcedureSchema,
  newProcedureServicesSchema,
...
import { useGetProcedureQuery, useUpdateProcedureMutation } from '@/lib/services/procedures';
...
export default function Page() {
  ...
  const [updateProcedure, { isLoading: isUpdatingProcedure }] =
  ...
  const onSubmit = async (data: z.infer<typeof newProcedureSchema>) => {
    try {
      const response = await updateProcedure({
        id: procedureId,
        body: data,
      }).unwrap();

      if (response.status === 'SUCCESS') {
        router.push(`/register/procedures/${procedureId}`);
        toast.custom((t) => (
          <CustomSonner
            t={t}
            description="Acto Clinico actualizado exitosamente"
            variant="success"
          />
        ));
      }
      ...
      toast.custom((t) => (
        <CustomSonner
          t={t}
          description="Error al actualizar el acto clinico"
          variant="error"
        />
      ));
    }
  ...
  useEffect(() => {
    if (procedureData) {
      form.reset({
        ...procedureData,
        created_by: procedureData.created_by?.id,
      });
    }
  }, [procedureData]);
  ...
  return (
    <Form {...form}>
      <Header title="Actualizar acto clinico">
        <Button
          type="submit"
          onClick={form.handleSubmit(onSubmit, onError)}
          size="sm"
          className="ml-auto"
          loading={isUpdatingProcedure}
        >
