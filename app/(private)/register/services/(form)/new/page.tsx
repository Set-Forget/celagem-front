...
import { useCreateServiceMutation } from '@/lib/services/services';
...
export default function Page() {
  ...
  const newServiceForm = useForm<z.infer<typeof newServiceSchema>>({
  ...
  const onSubmit = async (data: z.infer<typeof newServiceSchema>) => {
    try {
      ...
      if (response.status === 'SUCCESS') {
        router.push(`/register/services/${response.data.id}`);
        toast.custom((t) => (
          <CustomSonner
            t={t}
  ...
  useEffect(() => {
    if (profile) {
      newServiceForm.setValue('created_by', profile.data.id);
    }
  }, [profile]);
  ...
  return (
    <Form {...newServiceForm}>
      <Header title="Nuevo servicio">
        <Button
          type="submit"
          onClick={newServiceForm.handleSubmit(onSubmit, onError)}
          size="sm"
          className="ml-auto"
