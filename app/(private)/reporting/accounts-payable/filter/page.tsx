...
export default function Page() {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    if (suppliers && suppliers.length > 0) params.append("suppliers", suppliers);
    ...
    try {
      ...
      const filteredAccountsPayable = accountsPayable.data
        .filter(item => {
          if (suppliers && suppliers.length > 0) return suppliers.includes(String(item.partner.id))
          return true
        })
        .filter(item => {
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col w-full">
                  ...
                  {form.formState.errors.suppliers ? (
                    ...
                    <FormDescription>
                      Proveedores que figuran en los comprobantes.
                    </FormDescription>
                  }
                </FormItem>
              )}
