  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 p-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <span className={cn("text-sm transition-all duration-300", isInvoiceLoading || isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
          {isInvoiceLoading || isDebitNoteLoading ? placeholder(13) : invoice?.date ? format(parseISO(invoice?.date) ?? "", "dd MMM yyyy", { locale: es }) : "No especificado"}
        </span>
      </div>
      <div className="flex flex-col gap-1">
        <label className="text-muted-foreground text-sm">Fecha de vencimiento</label>
        <span className={cn("text-sm transition-all duration-300", isInvoiceLoading || isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
          {isInvoiceLoading || isDebitNoteLoading ? placeholder(13) : invoice?.due_date ? format(parseISO(invoice?.due_date) ?? "", "dd MMM yyyy", { locale: es }) : "No especificado"}
        </span>
      </div>
      <div className="flex flex-col gap-1">
