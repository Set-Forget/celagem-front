...
export default function DebitNotePage() {
  ...
  const status = debitNoteStatus[
    debitNote?.status === "posted" && new Date(debitNote?.due_date) < new Date()
      ? "overdue"
      : debitNote?.status as keyof typeof debitNoteStatus
  ];
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="flex flex-col gap-1">
              ...
              <span className={cn("text-sm transition-all duration-300", isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
                {isDebitNoteLoading ? placeholder(10) : debitNote?.partner || "No especificado"}
              </span>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-muted-foreground text-sm">Fecha de emisión</label>
              <span className={cn("text-sm transition-all duration-300", isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
                {isDebitNoteLoading ? placeholder(13) : format(debitNote?.date ?? "", "dd MMM yyyy", { locale: es })}
              </span>
            </div>
            {/* 
            // ! Ver esto mañana, si notas de débito tienen fecha de vencimiento y condicion de pago.
            */}
            {/*             <div className="flex flex-col gap-1">
              <label className="text-muted-foreground text-sm">Fecha de vencimiento</label>
              <span className={cn("text-sm transition-all duration-300", isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
                {isDebitNoteLoading ? placeholder(13) : format(debitNote?.due_date ?? "", "dd MMM yyyy", { locale: es })}
              </span>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-muted-foreground text-sm">Condición de pago</label>
              <span className={cn("text-sm transition-all duration-300", isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
                {isDebitNoteLoading ? placeholder(10) : debitNote?.payment_term || "No especificado"}
              </span> 
            </div>
            */}
            <div className="flex flex-col gap-1">
              ...
              <span className={cn("text-sm transition-all duration-300", isDebitNoteLoading ? "blur-[4px]" : "blur-none")}>
                {isDebitNoteLoading ? placeholder(10) : debitNote?.payment_method || "No especificado"}
              </span>
            </div>
          </div>
        </div>
        {/*         <DataTable
          data={debitNote?.items.map((item) => ({ ...item, currency: debitNote?.currency })) ?? []}
