...
export default function TableFooter({ append }: { append: (value: any) => void }) {
  ...
  const debitNoteAmount = subtotal + subtotalTaxes

  const invoiceAmount = invoice?.items.reduce((acc, item) => {
  ...
  const netAmount = invoiceAmount + debitNoteAmount
