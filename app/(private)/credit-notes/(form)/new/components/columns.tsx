...
const UnitPriceCell = ({ control, index }: { control: Control<z.infer<typeof newCreditNoteSchema>>; index: number }) => {
...
const SubtotalCell = ({ control, index }: { control: Control<z.infer<typeof newCreditNoteSchema>>; index: number }) => {
  const { data: currencies } = useListCurrenciesQuery()
  ...
  const subtotal = (quantity || 0) * Number(priceUnit || 0)
...
const TaxesCell = ({ control, index }: { control: Control<z.infer<typeof newCreditNoteSchema>>; index: number }) => {
  ...
  const handleSearchTax = async (query?: string) => {
    const typeTaxUse = billId ? "purchase" : "sale"
