...
export default function Page() {
  ...
  const { data: warehouseData } = useGetWarehouseQuery(warehouseId, {
    skip: !warehouseId,
  });

  const form = useForm<z.infer<typeof newWarehouseSchema>>({
  ...
  useEffect(() => {
    if (warehouseData) {
      form.reset({
        ...
        view_location: warehouseData.view_location.id,
        lot_stock_location: warehouseData.lot_stock_location.id,
      });
    }
  }, [warehouseData]);
  ...
  return (
    <Form {...form}>
      <Header title="Actualizar almacén">
        <Button
          type="submit"
