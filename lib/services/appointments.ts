import {
  ...
  NewAppointment,
} from '@/app/(private)/medical-management/calendar/schemas/appointments';
...
export const appointmentsApi = hcApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    createAppointment: builder.mutation<
      AppointmentDetailResponse,
      Omit<NewAppointment, 'attention_type'> & {
        start_time: string;
        end_time: string;
      }
    >({
    ...
    deleteAppointment: builder.mutation<
      { code: number; data: object; message: string; status: string },
      { id: string }
    >({
      query: ({ id }) => ({
        url: `appointment/${id}`,
    ...
    updateAppointment: builder.mutation<
      AppointmentDetailResponse,
      { id: string; body: Partial<NewAppointment> }
    >({
      query: ({ id, body }) => ({
        url: `appointment/${id}`,
        method: 'PUT',
